...
// Purpose: Iterative 256-point normalized radix-2 FFT engine for the Phase 2 analysis path.
// Contract: Matches the reference-model FFT scheduling order: bit-reversed input load, radix-2
//           DIT butterflies, forward twiddles, and rnd2 normalization after every butterfly.
...
#(
    ...
    input  logic signed [IN_W-1:0]       in_sample_i,
  ...
  import trecap_core_pkg::*;
    ...
    assign input_re_ext = {{(DATA_W-IN_W){in_sample_i[IN_W-1]}}, in_sample_i};
    assign input_im_ext = '0;
    ...
    assign out_bin_idx_o = output_count_q;
    ...
    trecap_twiddle_rom #(
        ...
        .INV_RE_FILE(""), .INV_IM_FILE(""),
        .USE_INVERSE_FILES(1'b0), .REGISTER_OUTPUT(1'b1),
        .FIXED_DIRECTION(1'b1), .INVERSE_DIRECTION(1'b0)
    ) u_twiddle_rom (
        .clk(clk), .rst_n(rst_n), .clear_i(clear_i),
        .valid_i(rst_n && !clear_i && (state_q == S_READ)),
        .inverse_i(1'b0), .index_i(twiddle_index_comb),
    ...
    trecap_fft_stage #(
        ...
        .FRAC_SHIFT(T_FRAC_F), .NORMALIZE_BY_2(1'b1), .REGISTER_OUTPUT(1'b1)
    ) u_butterfly_stage (
        .clk(clk), .rst_n(rst_n), .clear_i(clear_i),
                ...
                case (state_q)
                    ...
                    S_START: if (stage_ready) begin
                        if (!tw_valid || tw_oob || (tw_index != twiddle_index_comb) ||
                            (tw_inverse != 1'b0)) protocol_error_sticky_q <= 1'b1;
                        state_q <= S_WAIT;
                    end
...
`ifndef SYNTHESIS
    initial begin
        if (L != 256) begin
            $fatal(1, "trecap_fft256: this implementation expects L=256, got %0d", L);
        end
        if (P != 8) begin
            $fatal(1, "trecap_fft256: this implementation expects P=8, got %0d", P);
        end
        if (DATA_W < IN_W) begin
            $fatal(1, "trecap_fft256: DATA_W must be >= IN_W");
        end
    end
`endif

endmodule : trecap_fft256
