...
def read_json(path: str | Path) -> dict[str, Any]:
    """Read one strict RFC 8259 JSON object manifest.

    Python's default ``json.loads`` silently keeps the last duplicate member
    and accepts NaN/Infinity.  Both behaviors are unsafe for hash-bearing
    manifests because two readers can interpret the same bytes differently.
    """
    ...
    if raw.startswith(b"\xef\xbb\xbf"):
        raise ManifestError(f"{p}: UTF-8 BOM is forbidden")
    try:
        text = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ManifestError(f"{p}: JSON must be UTF-8: {exc}") from exc
    try:
        obj = json.loads(
            text,
...
def write_json(path: str | Path, obj: Mapping[str, Any]) -> None:
    ...
    p.write_text(
        json.dumps(dict(obj), indent=2, sort_keys=False) + "\n",
        encoding="utf-8",
        newline="\n",
    )
...
def artifact_index_entries(artifact_index_path: str | Path) -> dict[str, dict[str, Any]]:
    ...
    for entry in entries:
        ...
        if entry["path"] in out:
            raise ManifestError(f"artifact_index.json has duplicate path {entry['path']!r}")
