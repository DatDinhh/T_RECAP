...
def generate(
    ...
    *,
    ...
    """Generate a step: zero before ``index``, ``satN(amplitude)`` afterward."""
    ...
    if idx < 0 or idx > checked_ns:
        raise GeneratorError(f"step index must satisfy 0 <= index <= Ns, got {idx}")
    raw_amp = amplitude if amplitude is not None else A
    if raw_amp is None:
        raw_amp = 0
    amp = satn(coerce_int(raw_amp, name="amplitude"), width)
    return tuple(0 if n < idx else amp for n in range(checked_ns))
