...
```bash
make freeze-release CREATED_UTC=2026-07-24T06:10:00Z
```
...
```bash
make reproduce-release CREATED_UTC=2026-07-24T06:10:00Z
```

This target regenerates `artifact_index.json` and
`frozen_release_manifest.json` in a temporary artifact copy and compares both
files byte-for-byte with the committed release. Override `CREATED_UTC` only
when intentionally freezing a new release timestamp.
