...
The bundled `0.1.0-dev.1` artifacts are an LF-derived development release.
Their internal hash DAG is closed, but the original source ZIP is not
byte-verified. Do not use this bundle to claim original-archive provenance or
Phase 2 signoff.
...
| `make freeze-release` | Freeze artifact hashes using deterministic `CREATED_UTC` (v69 default: `2026-07-24T06:10:00Z`). |
| `make reproduce-release` | Re-freeze a temporary artifact copy and compare both DAG manifests byte-for-byte. |
...
The artifact checker validates `core_config_snapshot.json` against both its
Draft 2020-12 schema and the generated `spec/generated/core_config.json`
payload. Missing `jsonschema`, a missing schema, or a missing index/release
