...
## 0.1.0-dev.1-reference-integrity - 2026-07-24

### Fixed

- Rebuilt the LF-derived artifact index and frozen-release hash DAG instead of
  retaining raw hashes from the earlier CRLF byte representation.
- Made `artifact_check.py` require `jsonschema`, every schema, every frozen
  manifest, a complete artifact inventory, safe non-symlink paths, and every
  nested raw/canonical hash.
- Made `freeze_release.py` run an explicit pre-freeze content check and a
  mandatory post-freeze DAG check.
- Replaced the placeholder core-config snapshot with the generated
  `spec/generated/core_config.json` payload.

### Changed

- Bumped the bundled development release from `0.1.0-dev.0` to
  `0.1.0-dev.1`. This is an LF-derived development release, not a byte-identical
  reissue of the unavailable original archive.
