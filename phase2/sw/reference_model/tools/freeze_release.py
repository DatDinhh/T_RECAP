...
def vector_name_from_path(path: Path) -> str | None:
    parts = path.as_posix().split("/")
    for anchor in ("test_vectors", "golden", "reference_outputs"):
        if anchor in parts:
...
def schema_ref_for_json(path: Path) -> str:
    name = path.name
    if name == "core_config_snapshot.json":
        return "spec/schemas/core_config.schema.json"
...
def collect_artifact_paths(root: Path, include_index: bool) -> list[Path]:
    ...
    for path in paths:
        if path.name == "frozen_release_manifest.json":
            continue
        if path.name == "reference_import_manifest.json":
            continue
...
def write_core_config_snapshot(path: Path) -> None:
    source = Path("spec/generated/core_config.json")
    if not source.is_file():
        raise ToolError(f"generated core config is required before freezing: {source}")
    # This is a byte snapshot, not a second serializer.  The checker validates
    # it against core_config.schema.json and the generated Python payload.
...
def run() -> int:
    ...
    parser.add_argument(
        "--created-utc",
        default=None,
        help="Explicit reproducible UTC timestamp for generated manifests.",
    )
