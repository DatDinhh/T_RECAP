...
"""Cosine and exact-bin cosine input-vector generators."""
...
def generate(
    ...
    *,
    ...
    """Generate ``satN(qcoef(A*cos(2*pi*f*n + phase), 0))``."""
    ...
    return tuple(
        satn(qcoef(amp * math.cos((2.0 * math.pi * freq_num * n / freq_den) + phase), 0), width)
...
def generate_exact_bin(
    ...
    *,
    ...
    """Generate an exact-bin cosine using ``f = k/L``."""
