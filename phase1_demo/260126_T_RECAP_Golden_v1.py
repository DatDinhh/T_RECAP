...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.plot(x[:num_samples], label="x[n] (input)")
    plt.plot(y[:num_samples], label="y[n] (output)")
    plt.xlabel("Sample index n")
    plt.ylabel("Amplitude")
    plt.title(f"x[n] vs y[n] N={N} T={T} SHIFT={SHIFT} W={W} rseed={rseed}")
    plt.legend()
    plt.grid(True)
    plt.show()
